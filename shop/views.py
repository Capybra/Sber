...
from .models import Category, Product, StarRating, Comment, LunchPosition, CommentManage
from cart.cart import Cart
from staff.models import Staff
from .forms import CommentForm, StarRatingForm, ProductStarRatingUpdateForm, CommentManageForm
...
def print_us(request):
    if request.method == 'POST':
        form = CommentManageForm(request.POST)
        if form.is_valid():
            print(form.cleaned_data)
            form.save()
            return redirect('shop:about_us')
    else:
        # Если это GET-запрос, создайте форму без данных
        form = CommentManageForm()

    return render(request, 'shop/product/print_us.html', {'form': form})

def manage_us(request):
    #if not check_login(request):
      # return redirect(reverse("staff:login"))
    
    comments = CommentManage.objects.all()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                return redirect('staff:manage_us')
            except Exception as e:
                # Обработка ошибки сохранения (можете настроить вывод в лог или отправку уведомления)
                print(f"Error saving comment: {e}")

    else:
        # Если это GET-запрос, создаем пустую форму
        instance = comments.first() if comments.exists() else None
        form = CommentManageForm(instance=instance)

    return render(request, 'staff/manage_us.html', {'comments': comments, 'form': form})
